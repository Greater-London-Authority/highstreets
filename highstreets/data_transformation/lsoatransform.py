...
class LsoaTransform(DataLoader):
    def __init__(self):
        """
        LSOA Transform class for performing data transformation on data.

        This class provides methods to transform input data containing HSDS information,
        such as lsoa_id, date, total_volume, worker_population_percentage,
    ...
    def raw_lsoa_transform_data(self, data):
        """
        Transform raw LSOA (Lower Layer Super Output Area) data.

        Parameters:
        - data (list or dict): Raw LSOA data in JSON format. It should contain
        ...
        Returns:
        - pd.DataFrame: Transformed LSOA data in a pandas DataFrame format. The DataFrame
                        includes columns such as LSOA ID, LSOA name, count date, day,
        ...
        Example:
        ```python
        data = [...]  # Replace with actual raw LSOA data
        transformed_data = raw_lsoa_transform_data(data)
        ```
