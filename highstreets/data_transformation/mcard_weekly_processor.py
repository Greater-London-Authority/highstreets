...
class FileProcessor:
    ...
    def get_inner_outer_weekly_summary(self, since_date=None):
        ...
        if since_date:
            query = query.replace(
                "WHERE ", f"WHERE c.week_start >= '{since_date}' AND "
            )
    ...
    def process_inner_outer_weekly_summary(
        ...
        """
        # Get existing data to determine what's already processed (if 'since_date' not
        # specified)
        try:
            ...
            if not existing_data.empty:
                ...
                if process_full_data is False:
                    since_date = (
                        pd.to_datetime(max_date) + pd.Timedelta(days=1)
                    ).strftime("%Y-%m-%d")
                else:
        ...
        if not df.empty:
            try:
                ...
                if self.data_writer.table_exists(target_table):
                    # if table already exists and not processing full data,
                    # append to existing table
                    if process_full_data is False:
                        self.data_writer.append_data_to_postgres(
                            df, target_table, date_column="week_start"
                        )
                    else:
                        # If processing full data -> truncate and load
                        self.data_writer.truncate_and_load_to_postgres(
                            df,
                            target_table,
                            schema="gisapdata",
                ...
                if output_csv:
                    csv_path = (
                        f"{self.base_dir}mastercard/weekly/"
